  ...
  return (
    <div ref={userSettingsRef} className="relative flex items-center">
      <button
        ...
        className="min-w-10 cursor-pointer border-2 rounded-full hover:border-accent"
      >
        <Image
          src={avatar}
          alt="user"
      ...
      {openUserSettings && (
        <div className="absolute top-18 left-0 z-10 transition-all duration-200 ease-out">
          ...
          <div
            className="absolute z-10 -top-2.5 -left-5 min-w-40 mt-2 bg-card rounded-md shadow-lg ring-1 ring-border ring-opacity-5 focus:outline-none"
            role="menu"
            aria-hidden={!openUserSettings}
          >
            <div className="absolute z-10 -top-3 right-28 w-0 h-0 border-card border-l-8 border-r-8 border-b-8 border-l-transparent border-r-transparent border-b-black animate-bounce" />
            <div className="py-1">
              <Link
                href={`/profile`}
                className="inline-flex items-center gap-2.5 w-full px-4 py-2 text-xs hover:text-accent"
                role="menuitem"
              >
                <User size={22} />
                Perfil
              ...
              <button
                onClick={() => signOut()}
                className="inline-flex items-center gap-2.5 w-full px-4 py-2 text-xs text-start hover:text-accent cursor-pointer"
                role="menuitem"
              >
                <LogOut size={22} />
                Sair
